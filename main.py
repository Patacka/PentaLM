...
class PPOUpdater(BaseUpdater):
    ...
    def perform_update(self, contexts, candidates, _current_batch_ids, **kwargs):
        ...
        for k in ['advantages', 'returns', 'logprobs', 'values']:
        ...
        for step in range(n_minibatches):
            ...
            ratio = torch.exp(scores - current_process_buffer['logprobs'][_start_idx:_stop_idx])
            #print("ratio")
            #print(ratio)
...
def main(config: DictConfig):
    ...
    for epoch in range(config.rl_script_args.epochs):
        for t in tqdm(range(config.rl_script_args.steps_per_epoch), ascii=" " * 9 + ">", ncols=100):
            ...
            action = agent.generate([task_prompt], **config.lamorel_args.llm_args.generation_args)[ZERO_AS_BS_IS_1][0]['text']
            rl_script_logger.info(f"Generated action: {action}")
            rl_script_logger.info(f"Epoch Step: {t}")
            action = action.replace(task_prompt, '')
            lines = action.split('\n')
            action = '\n'.join(lines[1:]) if len(lines) > 1 else ""
            action = action.replace("<|im_end|>", '')

            score_and_value = agent.custom_module_fns(['score', 'value'], contexts=[task_prompt], candidates=[[action]])[ZERO_AS_BS_IS_1]
            observation, reward, done, _ = env.step(action)

            epoch_ended = t+1 == config.rl_script_args.steps_per_epoch
            buffer.store(task_prompt, [action], reward, score_and_value['value'][0], score_and_value['score'][0])
